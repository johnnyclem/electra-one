...
function luaUpload(luaStr) {
  // MIDI SysEx data bytes MUST be 0x00–0x7F. Node's 'ascii' encoding only
  // keeps the low 8 bits of each code unit, so characters like '·' (U+00B7)
  // become 0xB7 and corrupt the message — the device never ACKs and push hangs.
  // Force 7-bit and drop anything outside printable/control ASCII used in Lua.
  const payload = [];
  for (let i = 0; i < luaStr.length; i++) {
    const c = luaStr.charCodeAt(i);
    if (c === 0x09 || c === 0x0a || c === 0x0d || (c >= 0x20 && c <= 0x7e)) {
      payload.push(c);
    } else if (c > 0x7f) {
      // Best-effort: keep low 7 bits only for edge cases (e.g. already-sanitized).
      // Prefer pure-ASCII sources; non-ASCII in comments/strings becomes garbage.
      payload.push(c & 0x7f);
    }
